...
def run(cases,live=False,compile_sql=True):
    ...
    for case in cases:
        ...
        try:
            ...
            if compile_sql and validation_ok and not result.sql: result=engine.compile(result)
            ...
            passed=metric_ok and entity_ok and plan_ok and validation_ok and (sql_compiled and execution_ok if compile_sql else True)
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--dataset",default=str(DEFAULT_DATASET)); p.add_argument("--limit",type=int); p.add_argument("--live",action="store_true"); p.add_argument("--static",action="store_true",help="Skip LLM SQL compilation; validate deterministic planning stages only"); p.add_argument("--output",default="benchmark-report.json"); args=p.parse_args()
    ...
    results=run(load_cases(Path(args.dataset),args.limit),args.live,compile_sql=not args.static); report=summary(results)
