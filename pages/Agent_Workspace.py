"""Agent Workspace: inspectable agentic workflow with customer-facing answer intelligence."""
import json
import streamlit as st
from agentic.engine import AgenticQueryEngine
from agentic.answer import build_answer, render_answer

st.set_page_config(page_title="GenQuery Agent Workspace", page_icon="🧠", layout="wide")
st.title("🧠 GenQuery Agent Workspace")
st.caption("Retrieve → Resolve → Plan → Validate → Compile → Execute → Interpret → Prove")
...
if "answer_df" in st.session_state and st.session_state.get("answer_artifact"):
    view = st.selectbox(
        "View",
        ["auto", "executive", "kpi", "chart", "trend", "comparison", "insight", "table", "raw"],
        key="answer_view",
    )
    artifact = st.session_state["answer_artifact"]
    render_answer(artifact, st.session_state["answer_df"], view=view)
else:
    st.info("Run the validated SQL to generate an answer artifact.")
        ...
        try:
            ...
            if not all(SF_PARAMS.get(k) for k in ("user", "password", "account")):
                ...
                evidence.append({
                    "type": "execution",
                    "source": "snowflake",
                    "detail": f"{len(df)} rows returned",
                })
                artifact = build_answer(question, df, plan=r.plan, evidence=evidence)
                r.answer = artifact.to_dict()
                r.traces.append(type(r.traces[0])(
                    "interpret",
                    "Answer Intelligence Agent",
                    "completed",
                    f"Selected customer-facing {artifact.answer_type} view",
                    [artifact.answer_type, f"confidence={artifact.confidence:.2f}"],
                ))
                st.session_state.agent_result = r
                st.session_state["answer_df"] = df
                st.session_state["answer_artifact"] = artifact
                st.success(f"Query completed — {len(df)} rows interpreted into a customer answer.")
        except Exception as exc:
...
if r.answer:
    with st.expander("Answer artifact contract"):
        st.json(r.answer)
